...
def check_env_vars():
    ...
    print(f"DEEPSEEK_API_KEY 存在: {bool(deepseek_key)}")
    print(f"ZHIPU_API_KEY 存在: {bool(zhipu_key)}")

    return deepseek_key, zhipu_key


async def test_llm_connections():
    """测试两个LLM的连接"""
    print("正在检查环境变量...")
    deepseek_key, zhipu_key = check_env_vars()

    if not deepseek_key and not zhipu_key:
        print("错误: 没有找到DEEPSEEK_API_KEY或ZHIPU_API_KEY")
        return
