...
class ActAgent(AgentInterface):
    ...
    def _infer_next_page_url(self, current_url: str, next_page: int) -> Optional[str]:
        ...
        for param in offset_params:
            if param in query_params:
                ...
                vals = query_params[param]
                current_offset = int(vals[0]) if vals and vals[0].isdigit() else 0
                # next_page is 1-based page number; offset = (next_page - 1) * page_size
        ...
        if re.search(r'/p/\d+', path):
            new_path = re.sub(r'/p/\d+', f'/p/{next_page}', path)
            return urlunparse(parsed._replace(path=new_path))

        # 末尾数字（谨慎：数字 < 1000 且路径段 >= 2）
        path_parts = path.rstrip('/').split('/')
        if (len(path_parts) >= 2 and path_parts[-1].isdigit()
                and int(path_parts[-1]) < 1000):
    ...
    async def _discover_pagination_pattern(self, browser, current_url: str, next_page: int) -> Optional[str]:
        ...
        # 收集与当前页同路径的链接参数
        param_numbers: dict[str, list[int]] = {}
        for link in links:
            try:
                parsed_link = urlparse(link)
                if parsed_link.netloc != parsed_current.netloc:
                    continue
                if parsed_link.path != parsed_current.path:
                    continue
                link_params = parse_qs(parsed_link.query)
                current_params = parse_qs(parsed_current.query)
                for key, vals in link_params.items():
                    if vals and vals[0].isdigit():
                        num = int(vals[0])
                        # 必须与当前页参数值不同（说明是分页链接）
                        current_val = current_params.get(key, [None])[0]
                        if str(current_val) != vals[0]:
                            param_numbers.setdefault(key, []).append(num)
            except Exception:
                continue
        ...
        # 选出出现最多的参数名
        best_param = max(param_numbers, key=lambda k: len(param_numbers[k]))
        query_params = parse_qs(parsed_current.query, keep_blank_values=True)
        query_params[best_param] = [str(next_page)]
        new_query = urlencode(query_params, doseq=True)
